...
bool SDAccess::init(SemaphoreHandle_t mutex) {
    ...
    // Never format unknown or damaged media. A failed mount is reported and
    // existing card contents remain untouched for explicit recovery.
    bool ok = SD.begin(SDCard::CS, SPI, SD_SPI_FREQ, "/sd", 5, false);
